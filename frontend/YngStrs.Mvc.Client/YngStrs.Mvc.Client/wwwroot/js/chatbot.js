...
var SoGeYoung = {
    ...
    start: function () {
        $.ajax({
            url: 'https://localhost:5001/api/values/5'
        }).done(SoGeYoung.init);
    },
    chatbot: {
        answers: [],
    ...
    init: function (questions) {
        ...
        // Анимране на гравитациони вълни
        //TweenMax.staggerTo('#loading-circle-1,#loading-circle-2, #loading-circle-3', .3, {y:-15, repeat:-1, yoyo: true, ease: Power2.easeInOut }, .1);
        //Анимиране на искри
        //TweenMax.to('#robo-sparks', 7, {scaleX:1.1, x:-3, ease: Linear.easeNone });
        // Анимиране на антенките
        //TweenMax.to('#g865', 1.6, { rotation:5, transformOrigin:'10 78', repeat:-1, yoyo: true, ease: Linear.easeNone});
        //TweenMax.to('#g859', 1.6, { rotation:-5, transformOrigin:'10 78', repeat:-1, yoyo: true, ease: Linear.easeNone});
        //TweenMax.to('#irobo-background', 10, { scale: .93, transformOrigin: '50% 50%', repeat:-1, yoyo: true, ease: Linear.easeNone});
        //TweenMax.to('#robo-sparks', 2, {scaleY:1.2, y:-4, repeat: -1, yoyo: true});

        SoGeYoung.createViewFactory(mainQuestion);

        //new SVGAnimator("#robo-sparks", 100);
        // $('#g865, #g859').hover(lock(function(event) {
        //     TweenMax.fromTo(this, .2, {rotation: -3, transformOrigin:'10 78'}, { rotation:3, repeat:7, yoyo: true, ease: Power2.easeInOut });
        // }, 3000));
    },
    ...
    showSmile: function () {
        // TweenMax.to('#loading-inner', .4, {x: 80});
        // TweenMax.to('#smile-inner', .4, {x: 58 });
        // TweenMax.to('#error-inner', .4, {x: 0 });
    },
    showLoading: function () {
        // TweenMax.to('#loading-inner', .4, {x: 0});
        // TweenMax.to('#smile-inner', .4, {x: 0 });
        // TweenMax.to('#error-inner', .4, {x: 0 });
    },
    showError: function (repeat) {
        // if(repeat != false) {
        //     TweenMax.to('#smile-inner', .3, { x: -25, repeat: 1, yoyo: true, repeatDelay: .3, ease: Power2.easeInOut });
        //     TweenMax.to('#error-inner', .3, { x: -65.5, repeat: 1, yoyo: true, repeatDelay: .3, ease: Power2.easeInOut });
        // } else {
        //     TweenMax.to('#smile-inner', .4, {x: 0 });
        //     TweenMax.to('#loading-inner', .4, {x: 80 });
        //     TweenMax.to('#error-inner', .4, {x: -65.5 });
        // }
    },
    ...
    onComplete: function () {
        ...
        // checks if user is logged in to submit form + answers or only answers
            ...
            success: function (data) {
                ...
                try {
                    if (parsed.status === false && checkAnswers > 0) {
                        // SoGeYoung.fail();
                        // console.log(typeof checkAnswers,checkAnswers);
                        ...
                        $('.soge-answer').empty();
                        return SoGeYoung.animateQuestionText("Както кажеш! Ако си промениш мнението, знаеш къде да ме намериш. ;)");
                    } else {
                        // alert('success');
                        ...
                        $('.soge-answer').empty();
                        SoGeYoung.success();
                        /* if (SoGeYoung.chatbot.answers.length === 0){
                            $('.soge-answer').empty();
                            return SoGeYoung.animateQuestionText("Както кажеш! Ако си промениш мнението, знаеш къде да ме намериш. ;)");
                        } */
    ...
    success: function () {
        SoGeYoung.animateQuestionText("Отлично! Вашите отговори бяха изпратени към отдел Човешки Ресурси.");
    },
    fail: function () {
        SoGeYoung.animateQuestionText("В момента не можем да обработим вашите отговори!");
    }
};
...
/* function isLangQuestion(q) {
	return q.topic === "languages"
} */
function renderOptions(options) {
    ...
    var $btns = _.map(options, function (opt, index) {
        ...
        $btn.on('click', function () {
            ...
            // console.log(index)
            // console.log(SoGeYoung.chatbot.answers);
        });
    ...
    $container.sogeBtn(); // add some effects to it
}
...
// function renderLangQuestion (choices) {
// 	alert('lang question');
// 	console.log(choices);
// }
//renderCitiesSelect();
